...
import { Heart, ThumbsDown, MessageCircle, X, Image as ImageIcon, Send, FileText, BarChart3, Images, Repeat2, Users } from 'lucide-react'
...
export default function Feed({ perfil: perfilProp, onVerPerfil }) {
  ...
  const [modalLikes, setModalLikes] = useState(null) // { pubId, lista[] }
  ...
  async function cargarTodo() {
    ...
    const [pubs, encs, comps, repostData] = await Promise.all([
      ...
      supabase.from('reposts').select('publicacion_id, user_id, profiles(id, nombre, foto_perfil_url)').order('created_at', { ascending: false }).limit(50),
    ])
    ...
    if (comparaciones.length > 0) {
      ...
      const { data: misVotosFotoData } = await supabase
        .from('votos_comparacion').select('comparacion_id, voto')
        .in('comparacion_id', compIds).eq('user_id', usuario.id)
      const misVotosMap = {}
    ...
    todo.sort((a, b) => {
      const fechaA = a.tipo === 'repost' ? new Date(a.repost_at) : new Date(a.created_at)
      const fechaB = b.tipo === 'repost' ? new Date(b.repost_at) : new Date(b.created_at)
  ...
  async function publicar(e) {
    ...
    const { data: nuevaPub } = await supabase
      ...
      .select('*, profiles(nombre, carrera, foto_perfil_url)').single()
    if (nuevaPub) {
  ...
  async function crearEncuesta(e) {
    ...
    const { data: nuevaEnc } = await supabase
      ...
      .select('*, profiles(nombre, foto_perfil_url)').single()
    if (nuevaEnc) {
  ...
  async function abrirModalLikes(e, pubId) {
    e.stopPropagation()
    const count = reacciones[pubId]?.corazon || 0
    if (count === 0) return
    setModalLikes({ pubId, lista: [] })
    setCargandoLikes(true)
    const { data } = await supabase
      .from('reacciones')
      .select('profiles(id, nombre, carrera, foto_perfil_url)')
      .eq('publicacion_id', pubId)
      .eq('tipo', 'corazon')
    setCargandoLikes(false)
    const lista = (data || []).map(r => r.profiles).filter(Boolean)
    setModalLikes({ pubId, lista })
  }
  ...
  async function borrarComentario(comentarioId) {
    ...
    setComentarios(prev => {
      const nuevo = { ...prev }
      Object.keys(nuevo).forEach(key => { nuevo[key] = nuevo[key].filter(c => c.id !== comentarioId) })
      return nuevo
    })
  }
    ...
    <div>
        ...
        <div className="modal-overlay" onClick={() => setModalLikes(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ padding:'16px 20px', borderBottom:'1px solid var(--border-subtle)', display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
              ...
              <button onClick={() => setModalLikes(null)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--ink-tertiary)', display:'flex' }}>
                <X size={18} />
              </button>
            </div>
          ...
          <div style={{ animation:'slideDown 200ms var(--ease-out) both' }}>
            <div style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
              ...
              <form onSubmit={publicar} style={{ flex:1 }}>
                <textarea value={texto} onChange={e => setTexto(e.target.value)} placeholder="¿Que estas pensando?" rows={3}
      ...
      {cargando ? (
        ...
        items.map((item, idx) => {
          ...
          const pubId = item.id
          return (
            <div key={key} className="feed-card" style={{ animationDelay:`${idx*30}ms` }}>
              {/* Banner repost */}
              {item.tipo === 'repost' && (
              ...
              {(item.tipo === 'publicacion' || item.tipo === 'repost') && (
                <CardPublicacion
                  item={item} miReac={misReacciones[pubId]}
      ...
      <div style={{ display:'flex', gap:12, alignItems:'center' }}>
        ...
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
            <p className="author-name" onClick={() => onVerPerfil && item.profiles?.id && onVerPerfil(item.profiles.id)}
...
function CardPublicacion({ item, miReac, reacs, abierto, coms, textoComentario, onReaccionar, onToggleComentarios, onComentarChange, onComentar, iniciales, perfil, onBorrarComentario, usuario, yaReposteado, conteoReposts, onRepost, onAbrirLikes }) {
  ...
  return (
    <>
      ...
      {(reacs.corazon > 0 || reacs.nomeGusta > 0 || coms.length > 0 || conteoReposts > 0) && (
        <div style={{ padding:'0 18px 10px', display:'flex', gap:12, alignItems:'center', borderBottom:'1px solid var(--border-subtle)' }}>
          {reacs.corazon > 0 && (
            <button className="likes-counter" onClick={e => onAbrirLikes(e, pubId)}>
      ...
      <SeccionComentarios abierto={abierto} coms={coms} textoComentario={textoComentario}
        onComentarChange={onComentarChange} onComentar={e => onComentar(e, pubId)}
        iniciales={iniciales} perfil={perfil} onBorrarComentario={onBorrarComentario} usuario={usuario} />
    </>
  )
}
