...
import { Heart, ThumbsDown, MessageCircle, X, Image as ImageIcon, Send, FileText, BarChart3, Images, Repeat2 } from 'lucide-react'

const PAGE_SIZE = 10

function SkeletonCard() {
  return (
    <div style={{ background:'var(--surface-1)', border:'1px solid var(--border-subtle)', borderRadius:'var(--r-xl)', marginBottom:14, overflow:'hidden', padding:'14px 18px' }}>
      <div style={{ display:'flex', gap:12, alignItems:'center', marginBottom:14 }}>
        <div style={{ width:40, height:40, borderRadius:'50%', background:'var(--surface-3)', flexShrink:0 }} className="skeleton-pulse" />
        <div style={{ flex:1 }}>
          <div style={{ height:13, width:'40%', background:'var(--surface-3)', borderRadius:'var(--r-sm)', marginBottom:7 }} className="skeleton-pulse" />
          <div style={{ height:11, width:'60%', background:'var(--surface-3)', borderRadius:'var(--r-sm)' }} className="skeleton-pulse" />
        </div>
      </div>
      <div style={{ height:14, width:'90%', background:'var(--surface-3)', borderRadius:'var(--r-sm)', marginBottom:8 }} className="skeleton-pulse" />
      <div style={{ height:14, width:'70%', background:'var(--surface-3)', borderRadius:'var(--r-sm)', marginBottom:14 }} className="skeleton-pulse" />
      <div style={{ height:200, width:'100%', background:'var(--surface-3)', borderRadius:'var(--r-md)' }} className="skeleton-pulse" />
    </div>
  )
}
...
export default function Feed({ perfil: perfilProp, onVerPerfil }) {
  ...
  const [misReposts, setMisReposts] = useState({})
  const [conteoReposts, setConteoReposts] = useState({})
  ...
  useEffect(() => {
    ...
    if (centinelRef.current) observerRef.current.observe(centinelRef.current)
    return () => observerRef.current?.disconnect()
  }, [hayMas, cargandoMas, cargando, pagina])
  ...
  async function cargarPagina(numeroPagina, esInicio) {
    if (esInicio) setCargando(true)
    else setCargandoMas(true)

    const desde = numeroPagina * PAGE_SIZE
    const hasta = desde + PAGE_SIZE - 1

    const [pubs, encs, comps, repostData] = await Promise.all([
      supabase.from('publicaciones').select('*, profiles(id, nombre, carrera, foto_perfil_url)').order('created_at', { ascending: false }).range(desde, hasta),
      numeroPagina === 0 ? supabase.from('encuestas').select('*, profiles(id, nombre, foto_perfil_url)').order('created_at', { ascending: false }).limit(20) : { data: [] },
      numeroPagina === 0 ? supabase.from('comparaciones').select('*, profiles(id, nombre, foto_perfil_url)').order('created_at', { ascending: false }).limit(20) : { data: [] },
      numeroPagina === 0 ? supabase.from('reposts').select('publicacion_id, user_id, profiles(id, nombre, foto_perfil_url)').order('created_at', { ascending: false }).limit(50) : { data: [] },
    ])
    ...
    if ((pubs.data || []).length < PAGE_SIZE) setHayMas(false)
    else setHayMas(true)

    // Reposts solo en primera página
    if (numeroPagina === 0) {
      const misRepostsMap = {}
      const conteoMap = {}
      const repostItems = []
      ;(repostData.data || []).forEach(r => {
        if (r.user_id === usuario.id) misRepostsMap[r.publicacion_id] = true
        conteoMap[r.publicacion_id] = (conteoMap[r.publicacion_id] || 0) + 1
        const pubOriginal = publicaciones.find(p => p.id === r.publicacion_id)
        if (pubOriginal && r.user_id !== pubOriginal.user_id) {
          repostItems.push({ ...pubOriginal, tipo:'repost', repost_by:r.profiles, repost_at:r.created_at, _repost_key:`repost-${r.user_id}-${r.publicacion_id}` })
        }
      })
      setMisReposts(misRepostsMap)
      setConteoReposts(conteoMap)

      if (encuestas.length > 0) {
        const encIds = encuestas.map(e => e.id)
        const { data: votosEnc } = await supabase.from('votos_encuesta').select('encuesta_id, opcion_index, user_id').in('encuesta_id', encIds)
        const misVotosMap = {}; const conteosMap = {}
        ;(votosEnc || []).forEach(v => {
      ...
      if (comparaciones.length > 0) {
        ...
        const { data: misVotosFotoData } = await supabase.from('votos_comparacion').select('comparacion_id, voto').in('comparacion_id', compIds).eq('user_id', usuario.id)
        const misVotosMap = {}
        ;(misVotosFotoData || []).forEach(v => { misVotosMap[v.comparacion_id] = v.voto })
        setMisVotosFoto(misVotosMap)
      }

      if (publicaciones.length > 0) {
        const pubIds = publicaciones.map(p => p.id)
        const { data: reacs } = await supabase.from('reacciones').select('publicacion_id, tipo, user_id').in('publicacion_id', pubIds)
        const reacMap = {}; const misReacMap = {}
        ;(reacs || []).forEach(r => {
      ...
      const todo = [...publicaciones, ...encuestas, ...comparaciones, ...repostItems]
      todo.sort((a, b) => {
        const fa = a.tipo === 'repost' ? new Date(a.repost_at) : new Date(a.created_at)
        const fb = b.tipo === 'repost' ? new Date(b.repost_at) : new Date(b.created_at)
        return fb - fa
      })
      setItems(todo)
    } else {
      // Páginas siguientes: solo publicaciones nuevas
      if (publicaciones.length > 0) {
        const pubIds = publicaciones.map(p => p.id)
        const { data: reacs } = await supabase.from('reacciones').select('publicacion_id, tipo, user_id').in('publicacion_id', pubIds)
        const reacMap = {}; const misReacMap = {}
        ...
        setReacciones(prev => ({ ...prev, ...reacMap }))
        setMisReacciones(prev => ({ ...prev, ...misReacMap }))
      }
      setItems(prev => [...prev, ...publicaciones])
    }

    setPagina(numeroPagina)
    if (esInicio) setCargando(false)
  ...
  async function publicar(e) {
    ...
    if (imagen) {
      ...
      if (!error) { const { data } = supabase.storage.from('fotos').getPublicUrl(nombre); imagen_url = data.publicUrl }
    }
    const { data: nuevaPub } = await supabase.from('publicaciones').insert({ user_id: usuario.id, contenido: texto.trim(), imagen_url }).select('*, profiles(nombre, carrera, foto_perfil_url)').single()
    if (nuevaPub) {
      setItems(prev => [{ ...nuevaPub, tipo:'publicacion' }, ...prev])
      setReacciones(prev => ({ ...prev, [nuevaPub.id]: { corazon:0, nomeGusta:0 } }))
    }
    setTexto(''); setImagen(null); setModoComposer(null); setEnviando(false)
  }
  ...
  async function crearEncuesta(e) {
    ...
    setEnviando(true)
    const { data: nuevaEnc } = await supabase.from('encuestas').insert({ user_id: usuario.id, pregunta: pregunta.trim(), opciones: opcionesValidas }).select('*, profiles(nombre, foto_perfil_url)').single()
    if (nuevaEnc) setItems(prev => [{ ...nuevaEnc, tipo:'encuesta', conteos: opcionesValidas.map(() => 0), totalVotos:0 }, ...prev])
    setPregunta(''); setOpciones(['','']); setModoComposer(null); setEnviando(false)
  }
  ...
  async function subirComparacion(e) {
    ...
    const urlA = await subir(fotoA, 'A'); const urlB = await subir(fotoB, 'B')
    if (urlA && urlB) {
      const { data: nuevaComp } = await supabase.from('comparaciones').insert({ user_id: usuario.id, foto_a_url: urlA, foto_b_url: urlB }).select('*, profiles(nombre, foto_perfil_url)').single()
      if (nuevaComp) setItems(prev => [{ ...nuevaComp, tipo:'comparacion', votos_a:0, votos_b:0 }, ...prev])
    }
    setFotoA(null); setFotoB(null); setModoComposer(null); setEnviando(false)
  }
  ...
  async function toggleRepost(e, pubId) {
    e.stopPropagation()
    const yaReposteado = misReposts[pubId]
    if (yaReposteado) {
      await supabase.from('reposts').delete().eq('publicacion_id', pubId).eq('user_id', usuario.id)
      setMisReposts(prev => { const n = { ...prev }; delete n[pubId]; return n })
      setConteoReposts(prev => ({ ...prev, [pubId]: Math.max((prev[pubId]||1)-1, 0) }))
      setItems(prev => prev.filter(i => !(i.tipo==='repost' && i.id===pubId && i.repost_by?.id===usuario.id)))
    } else {
      await supabase.from('reposts').insert({ publicacion_id: pubId, user_id: usuario.id })
      setMisReposts(prev => ({ ...prev, [pubId]: true }))
      setConteoReposts(prev => ({ ...prev, [pubId]: (prev[pubId]||0)+1 }))
    }
  }

  async function abrirModalLikes(e, pubId) {
    e.stopPropagation()
    if (!reacciones[pubId]?.corazon) return
    setModalLikes({ pubId, lista:[] }); setCargandoLikes(true)
    const { data } = await supabase.from('reacciones').select('profiles(id, nombre, carrera, foto_perfil_url)').eq('publicacion_id', pubId).eq('tipo', 'corazon')
    setCargandoLikes(false)
    setModalLikes({ pubId, lista:(data||[]).map(r=>r.profiles).filter(Boolean) })
  }
  ...
  async function votarEncuesta(e, encuestaId, opcionIndex) {
    ...
    if (miVoto === opcionIndex) {
      ...
      setItems(prev => prev.map(item => {
        ...
        const c=[...item.conteos]; c[opcionIndex]=Math.max(c[opcionIndex]-1,0)
        return { ...item, conteos:c, totalVotos:Math.max(item.totalVotos-1,0) }
      }))
    } else {
      if (miVoto !== undefined) {
        await supabase.from('votos_encuesta').delete().eq('encuesta_id', encuestaId).eq('user_id', usuario.id)
        setItems(prev => prev.map(item => {
          if (item.id !== encuestaId || item.tipo !== 'encuesta') return item
          const c=[...item.conteos]; c[miVoto]=Math.max(c[miVoto]-1,0); return { ...item, conteos:c }
        }))
      }
      ...
      setItems(prev => prev.map(item => {
        if (item.id !== encuestaId || item.tipo !== 'encuesta') return item
        const c=[...item.conteos]; c[opcionIndex]+=1
        return { ...item, conteos:c, totalVotos:miVoto===undefined?item.totalVotos+1:item.totalVotos }
      }))
    }
  }
  ...
  async function votarFoto(e, compId, voto) {
    ...
    if (votoActual === voto) {
      ...
      setItems(prev => prev.map(item => { if (item.id!==compId||item.tipo!=='comparacion') return item; const campo=voto==='A'?'votos_a':'votos_b'; return {...item,[campo]:Math.max((item[campo]||1)-1,0)} }))
    } else {
      if (votoActual) {
        await supabase.from('votos_comparacion').delete().eq('comparacion_id', compId).eq('user_id', usuario.id)
        setItems(prev => prev.map(item => { if (item.id!==compId||item.tipo!=='comparacion') return item; const ca=votoActual==='A'?'votos_a':'votos_b'; return {...item,[ca]:Math.max((item[ca]||1)-1,0)} }))
      }
      ...
      setItems(prev => prev.map(item => { if (item.id!==compId||item.tipo!=='comparacion') return item; const campo=voto==='A'?'votos_a':'votos_b'; return {...item,[campo]:(item[campo]||0)+1} }))
    }
  }

  async function cargarComentarios(id) {
    const { data } = await supabase.from('comentarios').select('*, profiles(nombre, foto_perfil_url)').eq('publicacion_id', id).order('created_at', { ascending:true })
  ...
  async function borrarComentario(comentarioId) {
    ...
    setComentarios(prev => { const n={...prev}; Object.keys(n).forEach(k => { n[k]=n[k].filter(c=>c.id!==comentarioId) }); return n })
  }
  ...
  return (
    <div>
      <style>{`
        ...
        @keyframes skeletonShimmer { 0%{opacity:0.5} 50%{opacity:1} 100%{opacity:0.5} }
        ...
        .repost-banner { display:flex; align-items:center; gap:8px; padding:8px 18px; background:var(--surface-2); border-bottom:1px solid var(--border-subtle); font-size:12px; color:var(--ink-tertiary); font-weight:500; }
        .tipo-badge { display:inline-flex; align-items:center; gap:4px; padding:3px 9px; border-radius:var(--r-full); font-size:11px; font-weight:700; letter-spacing:0.02em; }
        .reac-btn { background:none; border:none; cursor:pointer; padding:7px 10px; border-radius:var(--r-md); display:flex; align-items:center; gap:6px; font-size:13px; font-weight:500; font-family:'DM Sans'; transition:transform 150ms ease,background 150ms ease,color 150ms ease; color:var(--ink-tertiary); }
        .reac-btn:hover { transform:scale(1.05); background:var(--surface-2); }
        ...
        .reac-btn.active-repost { color:var(--success); }
        .likes-counter { background:none; border:none; cursor:pointer; display:flex; align-items:center; gap:4px; font-size:13px; color:var(--ink-tertiary); padding:2px 4px; border-radius:var(--r-sm); transition:all 150ms ease; font-family:'DM Sans'; }
        .likes-counter:hover { color:var(--accent-bright); background:var(--accent-muted); }
        .delete-btn { background:none; border:none; cursor:pointer; padding:5px 7px; border-radius:var(--r-sm); color:var(--ink-muted); line-height:1; transition:color 150ms ease,background 150ms ease; }
        ...
        .composer-type-btn { flex:1; padding:9px 0; border:1px solid var(--border-subtle); border-radius:var(--r-md); cursor:pointer; font-family:'DM Sans'; font-size:12px; font-weight:600; background:var(--surface-2); color:var(--ink-tertiary); transition:all 150ms ease; display:flex; align-items:center; justify-content:center; gap:6px; }
        .composer-type-btn:hover { border-color:var(--accent); color:var(--accent-bright); background:var(--accent-muted); }
        ...
        .send-btn { padding:8px 16px; background:var(--accent); color:#fff; border:none; border-radius:var(--r-md); font-family:'DM Sans'; font-weight:600; font-size:13px; cursor:pointer; transition:all 150ms ease; display:flex; align-items:center; gap:6px; }
        .send-btn:hover { background:var(--accent-bright); transform:translateY(-1px); }
        .submit-btn { padding:9px 0; background:var(--accent); color:#fff; border:none; border-radius:var(--r-md); font-family:'DM Sans'; font-weight:600; font-size:14px; cursor:pointer; transition:all 150ms ease; box-shadow:0 2px 8px var(--accent-glow); }
        ...
        .modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:300; display:flex; align-items:center; justify-content:center; padding:24px; }
        .modal-box { background:var(--surface-1); border:1px solid var(--border-default); border-radius:var(--r-xl); width:100%; max-width:360px; max-height:70vh; overflow:hidden; display:flex; flex-direction:column; animation:modalIn 200ms var(--ease-out) both; }
        .like-row { display:flex; align-items:center; gap:12px; padding:10px 20px; border-bottom:1px solid var(--border-subtle); cursor:pointer; transition:background 150ms ease; }
        .like-row:last-child { border-bottom:none; }
        .like-row:hover { background:var(--surface-2); }
        @media (max-width:480px) { .btn-label { display:none; } }
      `}</style>

      {/* Modal likes */}
      {modalLikes && (
        <div className="modal-overlay" onClick={() => setModalLikes(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ padding:'16px 20px', borderBottom:'1px solid var(--border-subtle)', display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
              <h3 style={{ margin:0, fontSize:15, fontWeight:700, color:'var(--ink-primary)', display:'flex', alignItems:'center', gap:8 }}>
                <Heart size={16} fill="#f43f5e" stroke="#f43f5e" /> Me gusta
              </h3>
              <button onClick={() => setModalLikes(null)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--ink-tertiary)', display:'flex' }}><X size={18} /></button>
            </div>
            <div style={{ overflowY:'auto', flex:1 }}>
              {cargandoLikes ? (
                <div style={{ padding:'32px', textAlign:'center', color:'var(--ink-tertiary)', fontSize:13 }}>Cargando...</div>
              ) : modalLikes.lista.length === 0 ? (
                <div style={{ padding:'32px', textAlign:'center', color:'var(--ink-tertiary)', fontSize:13 }}>Sin resultados</div>
              ) : modalLikes.lista.map(p => (
                <div key={p.id} className="like-row" onClick={() => { onVerPerfil && onVerPerfil(p.id); setModalLikes(null) }}>
                  <Avatar texto={p.nombre?.[0]?.toUpperCase()||'?'} foto={p.foto_perfil_url} size={38} />
                  <div>
                    <p style={{ margin:0, fontWeight:600, color:'var(--ink-primary)', fontSize:14 }}>{p.nombre}</p>
                    <p style={{ margin:0, color:'var(--ink-tertiary)', fontSize:12 }}>{p.carrera}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Composer */}
      <div style={{ background:'var(--surface-1)', border:'1px solid var(--border-subtle)', borderRadius:'var(--r-xl)', padding:'16px 20px', marginBottom:14 }}>
        {!modoComposer && (
          <>
            <div style={{ display:'flex', gap:12, alignItems:'center', marginBottom:12 }}>
              <Avatar texto={iniciales} foto={perfil?.foto_perfil_url} size={38} />
              <button onClick={() => setModoComposer('post')} style={{ flex:1, padding:'9px 16px', textAlign:'left', background:'var(--surface-2)', border:'1px solid var(--border-subtle)', borderRadius:'var(--r-full)', cursor:'pointer', color:'var(--ink-muted)', fontFamily:'DM Sans', fontSize:14, transition:'all 150ms ease' }}
            ...
            <div style={{ display:'flex', gap:8 }}>
              {[{ id:'post', icon:FileText, label:'Publicacion' },{ id:'encuesta', icon:BarChart3, label:'Encuesta' },{ id:'fotos', icon:Images, label:'Comparar fotos' }].map(t => {
                const Icon = t.icon
                return <button key={t.id} onClick={() => setModoComposer(t.id)} className="composer-type-btn"><Icon size={14}/> {t.label}</button>
              })}
            </div>
          </>
        )}
          ...
          <div style={{ animation:'slideDown 200ms var(--ease-out) both' }}>
            <div style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
              ...
              <form onSubmit={publicar} style={{ flex:1 }}>
                <textarea value={texto} onChange={e => setTexto(e.target.value)} placeholder="¿Que estas pensando?" rows={3} style={{ width:'100%', border:'none', outline:'none', resize:'none', fontFamily:'DM Sans', fontSize:15, background:'transparent', color:'var(--ink-primary)', lineHeight:1.6 }} />
                {imagen && (
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', borderTop:'1px solid var(--border-subtle)', paddingTop:12, marginTop:8 }}>
                  ...
                  <div style={{ display:'flex', gap:8 }}>
                    ...
                    <button type="submit" disabled={enviando||(!texto.trim()&&!imagen)} className="submit-btn" style={{ width:'auto', padding:'7px 20px' }}>{enviando?'Publicando...':'Publicar'}</button>
                  </div>
                </div>
              </form>
            </div>
            ...
            <form onSubmit={crearEncuesta}>
              ...
              {opciones.map((op,i) => (
                <div key={i} style={{ display:'flex', gap:8, marginBottom:8 }}>
                  ...
                  {opciones.length > 2 && <button type="button" onClick={() => setOpciones(opciones.filter((_,j)=>j!==i))} className="delete-btn"><X size={14}/></button>}
                </div>
              ))}
              {opciones.length < 5 && <button type="button" onClick={() => setOpciones([...opciones,''])} style={{ width:'100%', padding:'8px 0', background:'none', border:'1px dashed var(--border-default)', borderRadius:'var(--r-md)', color:'var(--ink-tertiary)', fontFamily:'DM Sans', fontSize:13, cursor:'pointer', marginBottom:12 }}>+ Agregar opcion</button>}
              <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
                ...
                <button type="submit" disabled={enviando||!pregunta.trim()} className="submit-btn" style={{ width:'auto', padding:'7px 20px' }}>{enviando?'Publicando...':'Publicar encuesta'}</button>
              </div>
            </form>
          </div>
        )}
          ...
          <div style={{ animation:'slideDown 200ms var(--ease-out) both' }}>
            ...
            <form onSubmit={subirComparacion}>
              ...
              <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
                ...
                <button type="submit" disabled={!fotoA||!fotoB||enviando} className="submit-btn" style={{ width:'auto', padding:'7px 20px' }}>{enviando?'Subiendo...':'Publicar comparacion'}</button>
              </div>
            </form>
          </div>
        )}
      ...
      {cargando ? (
        ...
        <div style={{ textAlign:'center', padding:'64px 0', color:'var(--ink-tertiary)', fontSize:15 }}>Aun no hay publicaciones. Se el primero.</div>
      ) : (
        <>
          {items.map((item, idx) => {
            const key = item._repost_key || `${item.tipo}-${item.id}`
            const pubId = item.id
            return (
              <div key={key} className="feed-card" style={{ animationDelay:`${idx*20}ms` }}>
                {item.tipo === 'repost' && (
                  <div className="repost-banner">
                    <Repeat2 size={13} style={{ color:'var(--success)' }} />
                    <span style={{ color:'var(--success)', fontWeight:600 }}>{item.repost_by?.nombre}</span>
                    <span>compartio esto</span>
                  </div>
                )}
                <CardHeader item={item} usuario={usuario} onEliminar={eliminar} onVerPerfil={onVerPerfil} />
                {(item.tipo === 'publicacion' || item.tipo === 'repost') && (
                  <CardPublicacion item={item} miReac={misReacciones[pubId]}
                    reacs={reacciones[pubId]||{corazon:0,nomeGusta:0}}
                    abierto={comentariosAbiertos[pubId]} coms={comentarios[pubId]||[]}
                    textoComentario={textoComentario[pubId]||''}
                    onReaccionar={reaccionar} onToggleComentarios={toggleComentarios}
                    onComentarChange={t => setTextoComentario(prev=>({...prev,[pubId]:t}))}
                    onComentar={comentar} iniciales={iniciales} perfil={perfil}
                    onBorrarComentario={borrarComentario} usuario={usuario}
                    yaReposteado={misReposts[pubId]} conteoReposts={conteoReposts[pubId]||0}
                    onRepost={toggleRepost} onAbrirLikes={abrirModalLikes} />
                )}
                {item.tipo === 'encuesta' && (
          ...
          {/* Centinela infinite scroll */}
          <div ref={centinelRef} style={{ height:20 }} />

          {/* Skeleton al cargar más */}
          {cargandoMas && <SkeletonLoader count={2} />}

          {/* Fin del feed */}
          {!hayMas && !cargandoMas && items.length > 0 && (
            <div style={{ textAlign:'center', padding:'24px 0 8px', color:'var(--ink-muted)', fontSize:13 }}>
...
function CardHeader({ item, usuario, onEliminar, onVerPerfil }) {
  const config = {
    ...
    repost:     { label:'Publicacion', bg:'rgba(37,99,235,0.12)', color:'#3b82f6', dot:'#3b82f6' },
  }
  const c = config[item.tipo] || config.publicacion
  const ini = item.profiles?.nombre ? item.profiles.nombre.split(' ').map(n=>n[0]).join('').slice(0,2).toUpperCase() : '?'
  return (
      ...
      <div style={{ display:'flex', gap:12, alignItems:'center' }}>
        ...
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
            <p className="author-name" onClick={() => onVerPerfil && item.profiles?.id && onVerPerfil(item.profiles.id)} style={{ margin:0, fontWeight:700, color:'var(--ink-primary)', fontSize:14, cursor:onVerPerfil?'pointer':'default' }}>
          ...
          <p style={{ margin:0, color:'var(--ink-muted)', fontSize:12 }}>
            {item.profiles?.carrera && <span style={{ color:'var(--ink-tertiary)', fontWeight:500 }}>{item.profiles.carrera} · </span>}
            {new Date(item.created_at).toLocaleDateString('es-CO', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' })}
          </p>
        </div>
      </div>
      {item.user_id === usuario?.id && item.tipo !== 'repost' && (
...
function CardPublicacion({ item, miReac, reacs, abierto, coms, textoComentario, onReaccionar, onToggleComentarios, onComentarChange, onComentar, iniciales, perfil, onBorrarComentario, usuario, yaReposteado, conteoReposts, onRepost, onAbrirLikes }) {
  const pubId = item.id
  const tieneContadores = reacs.corazon > 0 || reacs.nomeGusta > 0 || coms.length > 0 || conteoReposts > 0
  return (
    <>
      <div style={{ padding:'0 18px 14px' }}>
        ...
        {item.imagen_url && <div style={{ borderRadius:'var(--r-md)', overflow:'hidden' }}><img src={item.imagen_url} alt="publicacion" style={{ width:'100%', maxHeight:380, objectFit:'cover', display:'block' }} /></div>}
      </div>
      {tieneContadores && (
        <div style={{ padding:'0 18px 10px', display:'flex', gap:12, alignItems:'center', borderBottom:'1px solid var(--border-subtle)' }}>
          {reacs.corazon > 0 && <button className="likes-counter" onClick={e => onAbrirLikes(e, pubId)}><Heart size={13} fill="#f43f5e" stroke="#f43f5e" /> {reacs.corazon}</button>}
          ...
          {conteoReposts > 0 && <span style={{ fontSize:13, color:'var(--ink-tertiary)', display:'flex', alignItems:'center', gap:4 }}><Repeat2 size={13}/> {conteoReposts}</span>}
          {coms.length > 0 && <span style={{ fontSize:13, color:'var(--ink-tertiary)', marginLeft:'auto' }}>{coms.length} comentario{coms.length>1?'s':''}</span>}
        </div>
      )}
      <div style={{ padding:'2px 10px 6px', display:'flex', gap:0, borderTop: tieneContadores?'none':'1px solid var(--border-subtle)', justifyContent:'space-between' }}>
        <button className={`reac-btn${miReac==='corazon'?' active-heart':''}`} onClick={e => onReaccionar(e, pubId, 'corazon')}>
          <Heart size={17} fill={miReac==='corazon'?'#f43f5e':'none'} stroke={miReac==='corazon'?'#f43f5e':'currentColor'}/> <span className="btn-label">Me gusta</span>
        </button>
        <button className={`reac-btn${miReac==='nomeGusta'?' active-dislike':''}`} onClick={e => onReaccionar(e, pubId, 'nomeGusta')}>
          <ThumbsDown size={17} fill={miReac==='nomeGusta'?'var(--accent-bright)':'none'} stroke={miReac==='nomeGusta'?'var(--accent-bright)':'currentColor'}/> <span className="btn-label">No me gusta</span>
        </button>
        <button className={`reac-btn${abierto?' active-comment':''}`} onClick={e => onToggleComentarios(e, pubId)}>
          <MessageCircle size={17} fill={abierto?'var(--accent-bright)':'none'} stroke={abierto?'var(--accent-bright)':'currentColor'}/> <span className="btn-label">Comentar</span>
        </button>
        <button className={`reac-btn${yaReposteado?' active-repost':''}`} onClick={e => onRepost(e, pubId)}>
          <Repeat2 size={17}/> <span className="btn-label">{yaReposteado?'Compartido':'Compartir'}</span>
        </button>
      </div>
      <SeccionComentarios abierto={abierto} coms={coms} textoComentario={textoComentario} onComentarChange={onComentarChange} onComentar={e => onComentar(e, pubId)} iniciales={iniciales} perfil={perfil} onBorrarComentario={onBorrarComentario} usuario={usuario} />
    </>
  )
}
...
function CardEncuesta({ item, miVoto, abierto, coms, textoComentario, onVotar, onToggleComentarios, onComentarChange, onComentar, iniciales, perfil, onBorrarComentario, usuario }) {
  ...
  return (
    <>
      ...
      <SeccionComentarios abierto={abierto} coms={coms} textoComentario={textoComentario} onComentarChange={onComentarChange} onComentar={e => onComentar(e, item.id)} iniciales={iniciales} perfil={perfil} onBorrarComentario={onBorrarComentario} usuario={usuario} />
    </>
  )
}
...
function CardComparacion({ item, miVoto, abierto, coms, textoComentario, onVotar, onToggleComentarios, onComentarChange, onComentar, iniciales, perfil, onBorrarComentario, usuario }) {
  ...
  return (
    <>
      <div style={{ padding:'0 18px 16px' }}>
        <p style={{ fontSize:12, color:'var(--ink-muted)', marginBottom:12 }}>{total} votos · {miVoto?'toca para cambiar':'toca para votar'}</p>
